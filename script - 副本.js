...
// ==================== 2. 连接 Supabase 云端大脑 (开启安全防线) ====================
// ⚠️ 必填：请在这里贴上你真正的 URL 和 Key！
const supabaseUrl = 'https://ekaeienirogrgkjxvwtc.supabase.co/rest/v1/';
const supabaseKey = 'sb_publishable_mLKLqxXbN75bhUnSxkkA5w_4mwKr0rQ';

let supabase = null;

// 自动安全检查，防止配置错误导致全盘崩溃
if (supabaseUrl && supabaseUrl !== '你的_Project_URL' && window.supabase) {
    try {
        supabase = window.supabase.createClient(supabaseUrl, supabaseKey);
    } catch (e) {
        console.error("Supabase初始化失败，请检查密匙:", e);
    }
} else {
    console.warn("🐾 小狗提示：Supabase 未配置或未正确引入，云端功能暂时关闭，但本地特效正常运作中！");
}
...
async function loadDiaries() {
    ...
    const { data, error } = await supabase.from('diaries').select('*').order('id', { ascending: false });
    ...
    data.forEach(item => {
        ...
        div.style.cssText = "border-bottom: 2px dashed #FF879A; padding: 15px 0; text-align: left; overflow: hidden;";
        div.innerHTML = `
            <button onclick="deleteItem('diaries', ${item.id}, loadDiaries)" style="${deleteBtnStyle}">删除 ✖</button>
            <p style="font-size: 13px; color: #888;">${item.date} 🐾</p>
            <p style="font-size: 16px; color: #333; margin-top: 6px;">${item.text}</p>
        `;
        list.appendChild(div);
...
if (addDiaryBtn) {
    addDiaryBtn.addEventListener('click', async function() {
        if (!supabase) return alert('云端未连接');
        ...
        input.value = '正在飞往云端... 🕊️';
        input.disabled = true;
        
        await supabase.from('diaries').insert([{ text: text, date: dateString }]);
...
async function loadAnniversaries() {
    ...
    data.forEach(item => {
        ...
        div.innerHTML = `
            ...
            <button onclick="deleteItem('anniversaries', ${item.id}, loadAnniversaries)" style="${deleteBtnStyle} margin-top: 10px;">删除 ✖</button>
        `;
        list.appendChild(div);
    });
}
...
if (addAnnBtn) {
    addAnnBtn.addEventListener('click', async () => {
        ...
        await supabase.from('anniversaries').insert([{ title: titleInput.value.trim(), date: dateInput.value }]);
        titleInput.value = ''; dateInput.value = '';
        loadAnniversaries();
    });
}

// ==================== 6. 云端回艺相册页 (memory.html) ====================
async function loadMemories() {
    ...
    data.forEach(item => {
        ...
        div.innerHTML = `
            ...
            <p class="photo-caption" style="text-align: left; position: relative;">
                ${item.caption}
                <button onclick="deleteItem('memories', ${item.id}, loadMemories)" style="${deleteBtnStyle} float: right; margin-top: -5px;">删除 ✖</button>
            </p>
        `;
        list.appendChild(div);
    });
...
if (addMemBtn) {
    addMemBtn.addEventListener('click', async () => {
        ...
        await supabase.from('memories').insert([{ url: url, caption: capInput.value.trim() || "未命名美好瞬间" }]);
        urlInput.value = ''; capInput.value = '';
        loadMemories();
    });
}
...
async function loadWishes() {
    ...
    const { data, error } = await supabase.from('wishes').select('*').order('id', { ascending: true });
    if (error) return console.error(error);
    ...
    if (data.length === 0) {
        ...
        await supabase.from('wishes').insert(defaultWishes);
        return loadWishes(); 
    }
    ...
    data.forEach(item => {
        ...
        div.innerHTML = `
            <label style="display: flex; align-items: center; cursor: pointer; color: ${textColor}; text-decoration: ${textDecoration}; flex: 1;">
                <input type="checkbox" ${item.checked ? 'checked' : ''} onchange="toggleWish(${item.id}, ${item.checked})" style="transform: scale(1.3); margin-right: 10px; accent-color: #FF879A;">
                <span>${item.text}</span>
            </label>
            <button onclick="deleteItem('wishes', ${item.id}, loadWishes)" style="${deleteBtnStyle} float: none; margin-left: 10px;">删除 ✖</button>
        `;
        list.appendChild(div);
    });
}
...
window.toggleWish = async function(id, currentStatus) {
    if (!supabase) return;
    await supabase.from('wishes').update({ checked: !currentStatus }).eq('id', id);
    loadWishes(); 
}
...
if (addWishBtn) {
    addWishBtn.addEventListener('click', async () => {
        ...
        await supabase.from('wishes').insert([{ text: text, checked: false }]);
        input.value = '';
        loadWishes();
    });
}
